...
export default function Navbar({ user, profile, refreshProfile }) {
  ...
  function openAuth(mode) { setAuthMode(mode); setShowAuth(true) }
  ...
  return (
    <>
      {/* Top bar — announcement + About link */}
      <div className="bg-brand-900 text-brand-200 text-center py-1.5 text-xs font-ui tracking-wide flex items-center justify-center gap-4">
        <span>✞ &nbsp; Sharing God's Word — In Light of the Word: Truth in every page.</span>
        <Link href="/about" className="text-brand-300 hover:text-white underline underline-offset-2 transition-colors font-semibold">
          About
        </Link>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
            ...
            {isAdmin && (
              <Link href="/admin"
                className={`px-4 py-2 rounded-lg font-ui text-sm font-semibold transition-all ${
                  router.pathname === '/admin' ? 'bg-brand-600 text-white' : 'bg-brand-50 text-brand-700 hover:bg-brand-100'
                }`}>
                🛡 Admin
              </Link>
            )}
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setDropdownOpen(v => !v)}
                  ...
                  <div className="w-7 h-7 rounded-full bg-brand-600 flex items-center justify-center text-white text-xs font-bold font-ui flex-shrink-0 overflow-hidden">
                    {avatarUrl ? <img src={avatarUrl} alt={firstName} className="w-full h-full object-cover" /> : avatarLetter}
                  </div>
                  <div className="flex flex-col leading-tight">
                    <span className="font-ui text-sm font-semibold text-brand-800 leading-none">{firstName}</span>
                    {profile?.role && profile.role !== 'member' && (
                      <span className="font-ui text-xs text-brand-500 leading-none mt-0.5 capitalize">{profile.role}</span>
                    )}
                  </div>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-[100] animate-fadeUp">
                    <Link href="/profile" onClick={() => setDropdownOpen(false)} className="flex items-center gap-3 px-4 py-2.5 hover:bg-brand-50 transition-colors group">
                      <div className="w-7 h-7 rounded-full bg-brand-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
                        {avatarUrl ? <img src={avatarUrl} alt={firstName} className="w-full h-full object-cover" /> : <HiUser className="text-brand-600" size={14} />}
                      </div>
                    ...
                    <Link href="/profile" onClick={() => setDropdownOpen(false)} className="flex items-center gap-3 px-4 py-2.5 hover:bg-brand-50 transition-colors font-ui text-sm text-slate-700">
                      <HiUser size={16} className="text-slate-400" /> My Profile
                    </Link>
                    {isAdmin && (
                      <Link href="/admin" onClick={() => setDropdownOpen(false)} className="flex items-center gap-3 px-4 py-2.5 hover:bg-brand-50 transition-colors font-ui text-sm text-slate-700">
                        <HiShieldCheck size={16} className="text-slate-400" /> Admin Panel
                      </Link>
                    )}
                    <hr className="my-1 border-slate-100" />
                    <button onClick={signOut} className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-red-50 transition-colors font-ui text-sm text-red-600">
              ...
              <>
                <button onClick={() => openAuth('login')} className="px-4 py-2 font-ui text-sm font-medium text-brand-700 hover:bg-brand-50 rounded-lg transition-all">Sign In</button>
                <button onClick={() => openAuth('register')} className="px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white font-ui text-sm font-semibold rounded-lg transition-all shadow-sm">Register</button>
              </>
            )}
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-blue-100 bg-white px-4 py-3 space-y-1 shadow-lg">
            ...
            <Link href="/about" onClick={() => setMenuOpen(false)}
              className={`block px-4 py-2.5 rounded-lg font-ui text-sm font-medium transition-all ${
                router.pathname === '/about' ? 'bg-brand-50 text-brand-700' : 'text-slate-700 hover:bg-slate-50'
              }`}>
              About
            </Link>
            {isAdmin && (
              <Link href="/admin" onClick={() => setMenuOpen(false)} className="block px-4 py-2.5 rounded-lg font-ui text-sm font-semibold text-brand-700 bg-brand-50">
                🛡 Admin Panel
              </Link>
            )}
            {user && (
            ...
            {user ? (
              ...
              <div className="flex gap-2 pt-1">
                <button onClick={() => { openAuth('login'); setMenuOpen(false) }} className="flex-1 py-2.5 border border-brand-200 text-brand-700 rounded-xl font-ui text-sm font-semibold">Sign In</button>
                <button onClick={() => { openAuth('register'); setMenuOpen(false) }} className="flex-1 py-2.5 bg-brand-600 text-white rounded-xl font-ui text-sm font-semibold">Register</button>
              </div>
            )}
          </div>
        )}
