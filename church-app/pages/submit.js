...
export default function SubmitPage({ user, profile, authReady }) {
  ...
  if (submitted) {
    return (
      <>
        <Head><title>Submitted — In Light of the Word</title></Head>
  ...
  if (loadingEdit) {
    return (
      <>
        <Head><title>Loading… — In Light of the Word</title></Head>
        <Navbar user={user} profile={profile} />
  ...
  return (
    <>
      <Head>
        <title>{isEditMode ? 'Edit Publication' : 'Submit Article'} — In Light of the Word</title>
      </Head>
      <Navbar user={user} profile={profile} />
