...
import { HiDownload, HiExternalLink, HiCalendar, HiArrowLeft, HiShare, HiHeart, HiChat, HiPaperAirplane, HiTrash } from 'react-icons/hi'
...
export default function PublicationPage({ user, profile, pub, related: initialRelated, error: serverError }) {
  ...
  useEffect(() => {
    ...
    fetchComments()
    checkLiked()
    // Only increment views on first load (pub.id changes), not on every auth change
    if (!user) {
      ;(async () => {
  ...
  }, [pub?.id, user?.id]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function handleLike() {
    ...
    if (liked) {
      ...
      if (user) {
        const { error } = await supabase.from('likes').delete().eq('publication_id', pub.id).eq('user_id', user.id)
        if (error) { setLiked(true); setLikeCount(c => c + 1); toast.error('Could not unlike') }
      } else {
        try {
      ...
      if (user) {
        ...
        if (error) { setLiked(false); setLikeCount(c => Math.max(0, c - 1)); toast.error('Could not like'); return }
      } else {
        ...
        if (error && error.code !== '23505') {
          setLiked(false); setLikeCount(c => Math.max(0, c - 1)); toast.error('Could not like'); return
        }
        try {
          const lp = JSON.parse(localStorage.getItem('likedPubs') || '[]')
  ...
  async function handleDeleteComment(commentId) {
    if (!user) { toast.error('Sign in to delete comments'); return }
    if (!confirm('Delete this comment?')) return
    const { error } = await supabase.from('comments').delete()
      .eq('id', commentId)
      .eq('author_id', user.id) // RLS: users can only delete their own
  ...
  return (
    <>
      <Head>
        <title>{pub.title} — In Light of the Word</title>
        <meta name="description" content={pub.summary || pub.title} />
        {/* Open Graph — these drive the image/title shown when the link is shared */}
        <meta property="og:title" content={pub.title} />
        <meta property="og:description" content={pub.summary || pub.title} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`${process.env.NEXT_PUBLIC_SITE_URL || ''}/publication/${pub.id}`} />
        ...
        {pub.cover_image_url && <meta property="og:image:width" content="1200" />}
        {pub.cover_image_url && <meta property="og:image:height" content="630" />}
        {!pub.cover_image_url && <meta property="og:image" content={`${process.env.NEXT_PUBLIC_SITE_URL || ''}/logo.png`} />}
        {/* Twitter card */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={pub.title} />
        <meta name="twitter:description" content={pub.summary || pub.title} />
        <meta name="twitter:image" content={pub.cover_image_url || `${process.env.NEXT_PUBLIC_SITE_URL || ''}/logo.png`} />
      </Head>
      <Navbar user={user} profile={profile} />
                          ...
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {isOwn && (
                              <button
                                ...
                                className="ml-auto opacity-0 group-hover:opacity-100 p-1 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all"
      ...
      <footer className="hero-pattern text-white mt-12 py-6 text-center">
        <p className="font-ui text-sm text-blue-300">
          © <span suppressHydrationWarning>{new Date().getFullYear()}</span> In Light of the Word Ministry &nbsp;·&nbsp;
