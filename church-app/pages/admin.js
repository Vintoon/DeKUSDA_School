...
export default function AdminPage({ user, profile, authReady, refreshProfile }) {
  ...
  // Daily Verse state
  const [verseDate, setVerseDate]         = useState('')  // set in useEffect to avoid SSR/client mismatch
  ...
  // Set today's date on the client only — avoids SSR/client timezone mismatch
  useEffect(() => {
    setVerseDate(new Date().toISOString().split('T')[0])
  }, [])
