...
export default function AdminPage({ user, profile, authReady, refreshProfile }) {
  ...
  async function startEditPub(pub) {
    setEditPub(pub)
    setEditForm({ title: pub.title || '', summary: pub.summary || '', category: pub.category || 'general', featured: pub.featured || false })
  }
  async function saveEditPub(e) {
    e.preventDefault()
    ...
    const { error } = await supabase.from('publications').update({
      ...
      category: editForm.category, featured: editForm.featured,
    }).eq('id', editPub.id)
    setEditSaving(false)
    if (error) { toast.error(error.message); return }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 bg-slate-50 border-b border-slate-200">
              <h3 className="font-display font-bold text-slate-800 text-lg">Edit Publication</h3>
              <button onClick={() => setEditPub(null)} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-200 rounded-xl transition-colors"><HiX size={18} /></button>
            </div>
            <form onSubmit={saveEditPub} className="p-6 space-y-4">
              <div>
                <label className="font-ui text-xs font-semibold text-slate-500 mb-1 block">Title *</label>
                <input value={editForm.title} onChange={e => setEditForm(p => ({ ...p, title: e.target.value }))}
                  className="w-full border border-slate-200 rounded-xl px-3 py-2.5 font-ui text-sm outline-none focus:border-brand-400" />
              </div>
              <div>
                <label className="font-ui text-xs font-semibold text-slate-500 mb-1 block">Summary</label>
                <textarea value={editForm.summary} onChange={e => setEditForm(p => ({ ...p, summary: e.target.value }))}
                  rows={3} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 font-ui text-sm outline-none focus:border-brand-400 resize-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-ui text-xs font-semibold text-slate-500 mb-1 block">Category</label>
                  <select value={editForm.category} onChange={e => setEditForm(p => ({ ...p, category: e.target.value }))}
                    className="w-full border border-slate-200 rounded-xl px-3 py-2.5 font-ui text-sm outline-none focus:border-brand-400 bg-white">
                    {['devotional','sermon','study','prophecy','music','family','poetry','testimony','news','Health','review','general'].map(c => (
                ...
                <div className="flex items-end pb-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="font-ui text-sm font-medium text-slate-700">Featured ★</span>
                  </label>
                </div>
              </div>
              <div className="flex items-center gap-3 pt-2">
                ...
                <button type="button" onClick={() => setEditPub(null)} className="px-4 py-2.5 font-ui text-sm text-slate-500 hover:text-slate-700 rounded-xl hover:bg-slate-100 transition-colors">
                  Cancel
                </button>
              </div>
            </form>
                          ...
                          <tr key={pub.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => startEditPub(pub)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit"><HiPencil size={15} /></button>
