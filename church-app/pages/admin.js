...
export default function AdminPage({ user, profile, authReady, refreshProfile }) {
  ...
  const [verseForm, setVerseForm]       = useState({ verse_text:'', verse_reference:'', verse_date: new Date().toISOString().split('T')[0] })
  ...
  async function submitVerse(e) {
    e.preventDefault()
    if (!verseForm.verse_text || !verseForm.verse_reference) {
      toast.error('Verse text and reference are required'); return
    }
    setVerseSubmitting(true)
    const { error } = await supabase.from('daily_verse').insert({
      verse_text:      verseForm.verse_text.trim(),
      verse_reference: verseForm.verse_reference.trim(),
      verse_date:      verseForm.verse_date || new Date().toISOString().split('T')[0],
      created_by:      user.id,
    })
    setVerseSubmitting(false)
    if (error) {
      if (error.code === '23505') toast.error('A verse already exists for that date — delete it first.')
      else toast.error(error.message)
      return
    }
    toast.success('✝ Daily verse saved!')
    setVerseForm({ verse_text:'', verse_reference:'', verse_date: new Date().toISOString().split('T')[0] })
    loadVerses()
  }
  async function deleteVerse(id) {
    if (!confirm('Delete this verse?')) return
    const { error } = await supabase.from('daily_verse').delete().eq('id', id)
    if (error) { toast.error(error.message); return }
  ...
  const NAV = [
    ...
    { key:'dailyverse',   icon:'📅', label:'Daily Verse' },
  ...
  return (
    <>
          ...
          {NAV.map(s => (
            <button key={s.key}
              onClick={() => {
                ...
                if (s.key === 'dailyverse')    loadVerses()
              }}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {section === 'dailyverse' && (
          <div className="max-w-2xl">
            <h2 className="font-display text-xl font-bold text-slate-800 mb-2">📅 Daily Verse</h2>
            <p className="font-ui text-sm text-slate-500 mb-6">
              The verse you add here appears in the <strong>Quote of the Day popup</strong> for all visitors that day.
              One verse per date — the popup shows today's verse automatically.
            </p>

            {/* Add verse form */}
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-8">
              <h3 className="font-ui font-bold text-slate-700 mb-4 flex items-center gap-2">
                <HiPlusCircle size={16} className="text-amber-500" /> Add a Verse
              </h3>
              <form onSubmit={submitVerse} className="space-y-4">
                <div>
                  <label className="font-ui text-xs font-semibold text-slate-500 mb-1 block">
                    Verse Text *
                  </label>
                  <textarea
                    value={verseForm.verse_text}
                    onChange={e => setVerseForm(p => ({ ...p, verse_text: e.target.value }))}
                    placeholder="For God so loved the world, that he gave his only begotten Son…"
                    rows={3}
                    className="w-full border border-slate-200 rounded-xl px-3 py-2.5 font-body text-sm outline-none focus:border-amber-400 resize-none leading-relaxed"
                  />
                </div>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    <label className="font-ui text-xs font-semibold text-slate-500 mb-1 block">
                      Reference * <span className="font-normal text-slate-400">(e.g. John 3:16)</span>
                    </label>
                    <input
                      value={verseForm.verse_reference}
                      onChange={e => setVerseForm(p => ({ ...p, verse_reference: e.target.value }))}
                      placeholder="John 3:16"
                      className="w-full border border-slate-200 rounded-xl px-3 py-2.5 font-ui text-sm outline-none focus:border-amber-400"
                    />
                  </div>
                  <div>
                    <label className="font-ui text-xs font-semibold text-slate-500 mb-1 block">
                      Date <span className="font-normal text-slate-400">(defaults to today)</span>
                    </label>
                    <input
                      type="date"
                      value={verseForm.verse_date}
                      onChange={e => setVerseForm(p => ({ ...p, verse_date: e.target.value }))}
                      className="w-full border border-slate-200 rounded-xl px-3 py-2.5 font-ui text-sm outline-none focus:border-amber-400"
                    />
                  </div>
                </div>
                ...
                {/* Live preview */}
                {(verseForm.verse_text || verseForm.verse_reference) && (
                  <div className="bg-gradient-to-br from-brand-50 to-indigo-50 border border-brand-100 rounded-2xl px-5 py-4">
                    <div className="font-ui text-xs font-bold text-brand-500 uppercase tracking-widest mb-2">Preview</div>
                    <p className="font-body text-slate-800 italic leading-relaxed">
                      "{verseForm.verse_text || '…'}"
                    </p>
                    {verseForm.verse_reference && (
                      <p className="font-ui text-xs font-bold text-brand-600 mt-2 text-right">— {verseForm.verse_reference}</p>
                    )}
                  </div>
                )}
                ...
                <button
                  type="submit"
                  disabled={verseSubmitting}
                  className="flex items-center gap-2 px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-ui text-sm font-semibold transition-all disabled:opacity-60"
                >
                  📅 {verseSubmitting ? 'Saving…' : 'Save Verse'}
                </button>
              </form>
            </div>

            {/* Verse list */}
            <h3 className="font-ui font-bold text-slate-600 mb-3 text-sm uppercase tracking-wide">
              Recent Verses
            </h3>
            {versesLoading ? (
              <div className="space-y-2">
              ...
              <div className="text-center py-10 text-slate-400 font-ui text-sm">No verses added yet.</div>
            ) : (
              <div className="space-y-2">
                {verses.map(v => {
                  const today = new Date().toISOString().split('T')[0]
                  const isToday = v.verse_date === today
                  const isFuture = v.verse_date > today
                  return (
                    <div key={v.id}
                      className={`bg-white border rounded-xl px-4 py-3 flex items-start gap-3 ${isToday ? 'border-amber-300 bg-amber-50/40' : 'border-slate-100'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                          <span className="font-ui font-bold text-sm text-brand-700">{v.verse_reference}</span>
                          {isToday && (
                            <span className="font-ui text-xs font-bold bg-amber-400 text-white px-2 py-0.5 rounded-full">Today</span>
                          )}
                          {isFuture && (
                            <span className="font-ui text-xs font-bold bg-indigo-100 text-indigo-600 px-2 py-0.5 rounded-full">Scheduled</span>
                          )}
                          <span className="font-ui text-xs text-slate-400 flex items-center gap-1">
                            <HiCalendar size={10} /> {v.verse_date}
                          </span>
                        </div>
                        <p className="font-body text-sm text-slate-600 italic leading-relaxed line-clamp-2">
                          "{v.verse_text}"
                        </p>
                      </div>
                      <button
                        onClick={() => deleteVerse(v.id)}
                        className="p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0 mt-0.5"
                        title="Delete"
                      >
                        <HiTrash size={14} />
                      </button>
            ...
            <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-xl">
              <p className="font-ui text-xs text-slate-600 leading-relaxed">
                <strong className="text-brand-800">💡 How it works:</strong> Each day the site automatically shows the verse for that date in the
                Quote of the Day popup. You can schedule verses days or weeks in advance — just set a future date.
                Only one verse per date is allowed.
              </p>
            </div>
          </div>
        )}
