...
const CAT_LABELS  = {
  all:'All', about:'About', devotional:'Devotionals',sermon:'Sermons',study:'Bible Studies',Health:'Health',
  testimony:'Testimonies',  news:'Church News', review:'Book Reviews'
}
...
export default function Home({ user, profile }) {
  ...
  const loadPublications = useCallback(async (cat) => {
    setLoading(true)
    setFetchError(false)
    // 10-second timeout — if Supabase doesn't respond, show error + retry button
          ...
          <div className="min-w-0">
            ...
            {loading ? (
              ...
              <div className="text-center py-16 px-4">
                ...
                <button
                  onClick={() => loadPublications(category)}
                  className="inline-flex items-center justify-center px-6 py-3 bg-brand-600 text-white rounded-xl font-ui font-semibold text-sm hover:bg-brand-700 transition-colors min-h-[44px]">
                  🔄 Retry
                </button>
              </div>
            ) : filtered.length === 0 ? (
