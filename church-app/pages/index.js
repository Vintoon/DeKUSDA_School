...
export default function Home({ user, profile, initialPublications = [], initialStats = { total:0, sermons:0, contributors:0 } }) {
  // Start with server-pre-fetched data — no skeleton on first load
  const [publications, setPublications] = useState(initialPublications)
  const [loading, setLoading]           = useState(false)   // false because we have data already
  ...
  return (
    <div className="w-full overflow-x-hidden">
      <Head>
        <title>Silent Witness: Truth in every page</title>
          ...
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <div className="font-ui text-xs text-blue-300 mt-0.5">Silent Witness: Truth in every page</div>
              </div>
            </div>
...
// This is THE fix for the mobile loading issue.
//
// Previously: page was auto-exported as fully static (no data). Mobile had to
// download JS → hydrate → fetch Supabase → render. Any slow step = blank screen.
//
// Now: Vercel fetches publications at BUILD TIME and bakes them into the HTML.
// Mobile users see real publication cards the instant the page loads — before
// any JavaScript runs. revalidate:60 means new posts appear within 60 seconds.
// ─────────────────────────────────────────────────────────────────────────────
export async function getStaticProps() {
  try {
    ...
    const initialStats = {
      ...
      sermons: statsData?.filter(p => p.category === 'sermon').length || 0,
      contributors: new Set(statsData?.map(p => p.author_id).filter(Boolean) || []).size,
    }
    ...
    return {
      ...
      revalidate: 60, // Rebuild this page every 60 seconds so new posts appear quickly
    }
  } catch (e) {
    console.error('getStaticProps error:', e)
    return {
      props: { initialPublications: [], initialStats: { total:0, sermons:0, contributors:0 } },
      revalidate: 30,
    }
  }
}
