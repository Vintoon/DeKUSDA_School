...
const CATEGORIES = ['all','devotional','sermon','study','prophecy','music','family','poetry','testimony','news','Health','review']
const CAT_LABELS  = {
  all:'All', devotional:'Devotionals', sermon:'Sermons',
  study:'Bible Studies', prophecy:'Prophecy', music:'Music',
  family:'Family Life', poetry:'Poetry', testimony:'Testimonies',
  news:'Church News', Health:'Health', review:'Book Reviews'
}
...
export default function Home({ user, profile, initialPublications, initialStats }) {
  ...
  useEffect(() => {
    ...
    supabase.from('devotionals').select('*')
      .order('published_date', { ascending: false })
      .limit(1)
      .maybeSingle()
      .then(({ data }) => { if (data) setDevotional(data) })
  }, [])
  ...
  return (
    <div className="w-full overflow-x-hidden">
      <Head>
        <title>In Light of the Word — Truth in every page | DeKUSDA</title>
        <meta name="description" content="In Light of the Word — DeKUSDA's Christian publication ministry. Sermons, devotionals, Bible studies and testimonies freely shared to proclaim God's Word to the world." />
        <meta name="keywords" content="In Light of the Word, in light of the word, DeKUSDA, SDA church publications, Christian devotionals, Sabbath sermons, Bible studies, testimonies" />
        <link rel="canonical" href={process.env.NEXT_PUBLIC_SITE_URL || 'https://dekusdaschool.vercel.app'} />
        <link rel="icon" href="/logo.png" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        {/* Open Graph */}
        <meta property="og:title" content="In Light of the Word — Truth in every page | DeKUSDA" />
        <meta property="og:description" content="In Light of the Word — DeKUSDA's Christian publication ministry. Sermons, devotionals, Bible studies and testimonies freely shared." />
        <meta property="og:image" content={`${process.env.NEXT_PUBLIC_SITE_URL || ''}/logo.png`} />
        <meta property="og:url" content={process.env.NEXT_PUBLIC_SITE_URL || ''} />
        <meta property="og:type" content="website" />
        {/* Twitter */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="In Light of the Word — Truth in every page" />
        <meta name="twitter:description" content="In Light of the Word — DeKUSDA's Christian publication ministry. Sermons, devotionals, Bible studies and testimonies freely shared." />
        {/* JSON-LD — helps Google understand the site name and organisation */}
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify({
            "@context": "https://schema.org",
            "@type": "Organization",
            "name": "In Light of the Word",
            "alternateName": ["In Light of the Word DeKUSDA", "DeKUSDA In Light of the Word"],
            "url": process.env.NEXT_PUBLIC_SITE_URL || "https://dekusdaschool.vercel.app",
            "logo": `${process.env.NEXT_PUBLIC_SITE_URL || "https://dekusdaschool.vercel.app"}/logo.png`,
            "description": "In Light of the Word is DeKUSDA's Christian publication platform — sermons, devotionals, testimonies and Bible studies freely shared to proclaim God's Word.",
            "sameAs": []
          })}}
        />
        <script
          ...
          dangerouslySetInnerHTML={{ __html: JSON.stringify({
            ...
            "potentialAction": {
              ...
              "target": {
                "@type": "EntryPoint",
                "urlTemplate": `${process.env.NEXT_PUBLIC_SITE_URL || "https://dekusdaschool.vercel.app"}/?q={search_term_string}`
              },
      ...
      <div id="publications" className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-10">
        ...
        {devotional && (
          <div className="lg:hidden mb-5">
            <Link href="/devotional" className="block group">
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="h-1 w-full bg-gradient-to-r from-brand-700 via-brand-500 to-indigo-500" />
                <div className="px-4 py-4 flex items-start gap-3">
                  <div className="w-9 h-9 rounded-xl bg-brand-50 border border-brand-100 flex items-center justify-center text-lg flex-shrink-0">📖</div>
                  <div className="flex-1 min-w-0">
                    <div className="font-ui text-[10px] font-bold text-brand-600 uppercase tracking-widest mb-0.5">Today's Devotional</div>
                    <div className="font-display font-bold text-slate-800 text-sm leading-snug group-hover:text-brand-700 transition-colors">{devotional.title}</div>
                    {devotional.verse_reference && (
                      <div className="font-ui text-xs text-brand-500 mt-0.5">{devotional.verse_reference}</div>
                    )}
                  </div>
                  <div className="font-ui text-xs font-bold text-brand-600 flex-shrink-0 pt-1">Read →</div>
                </div>
              </div>
            </Link>
          </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="px-5 py-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="font-ui text-xs font-bold text-brand-700 uppercase tracking-wide">Today's Devotional</span>
                    </div>
                    <h4 className="font-display font-bold text-slate-800 text-base leading-snug mb-2 group-hover:text-brand-700 transition-colors">
                      {devotional.title}
                    </h4>
                    {devotional.verse && (
                      <p className="font-body text-sm text-slate-500 italic leading-relaxed line-clamp-2">
                        "{devotional.verse}"
                        {devotional.verse_reference && <span className="not-italic font-ui font-semibold text-brand-600 text-xs ml-1">— {devotional.verse_reference}</span>}
                      </p>
                    )}
                    {!devotional.verse && devotional.content && (
                      <p className="font-body text-sm text-slate-500 leading-relaxed line-clamp-3">{devotional.content}</p>
                    )}
                    <div className="mt-3 flex items-center gap-1 font-ui text-xs font-bold text-brand-600 group-hover:gap-2 transition-all">
                      Read full devotional <span>→</span>
                    </div>
                  </div>
                </div>
              </Link>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 grid grid-cols-1 sm:grid-cols-3 gap-8">
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <div className="font-display font-bold text-base leading-none">In Light of the Word</div>
